...
struct css_style_rec_tag
{
    ...
    bool isImportant(css_style_rec_important_bit bit) {
        return important[bit >> 5] & (1 << (bit & 0x1f));
    }
    void setImportant(css_style_rec_important_bit bit) {
        important[bit >> 5] |= (1 << (bit & 0x1f));
    }
    // apply value to field if important bit not yet set, then set it if is_important
    template <typename T>
    inline void Apply(T value, T* field, css_style_rec_important_bit bit, lUInt8 is_important) {
        ...
        if (!(important[slot] & sbit)                              // important flag not previously set
            ...
            *field = value; // apply
            if (is_important & 0x1)
                important[slot] |= sbit; // update important flag
            if (is_important == 0x3)
                importance[slot] |= sbit; // update importance flag (!important comes from higher_importance CSS)
        }
    }
    ...
    inline void ApplyAsBitmapOr(css_length_t value, css_length_t* field, css_style_rec_important_bit bit, lUInt8 is_important) {
        ...
        if (!(important[slot] & sbit) || (is_important == 0x3) || (is_important == 0x1 && !(importance[slot] & sbit))) {
...
{
public:
    lvdomStyleCache(int size = LV_STYLE_HASH_SIZE)
            : LVRefCache<css_style_ref_t>(size) { }
};
...
public:
    lvdomElementFormatRec()
            ...
            , _inner_y(0)
            , _baseline(0)
            , _usable_left_overflow(0)
            , _usable_right_overflow(0)
            , _top_overflow(0)
            , _bottom_overflow(0)
            , _lang_node_idx(0)
...
/// calculate cache record hash
inline lUInt32 calcHash(css_style_ref_t& rec) {
    return rec.isNull() ? 0 : calcHash(*rec.get());
}
