...
#if 0
#define REQUEST_RENDER(txt)                       \
    {                                             \
        CRLog::trace("request render from " txt); \
        requestRender();                          \
    }
#else
...
#if 0
#define CHECK_RENDER(txt)                                     \
    {                                                         \
        CRLog::trace("LVDocView::checkRender() - from " txt); \
        checkRender();                                        \
    }
#else
...
#define PGHDR_TEXT_ICONS_MASK (PGHDR_PAGE_NUMBER | PGHDR_PAGE_COUNT | PGHDR_AUTHOR | PGHDR_TITLE | \
                               ...
                               PGHDR_CHAPTER_NAME | PGHDR_CHAPTER_PAGE_NUMBER | PGHDR_CHAPTER_PAGE_COUNT)
...
void LVDocView::drawBatteryState(LVDrawBuf* drawbuf, const lvRect& batteryRc) {
    ...
    LVDrawBatteryIcon(drawbuf, batteryRc, m_battery_charge_level, m_battery_state == CR_BATTERY_STATE_CHARGING, icons, drawPercent ? m_batteryFont.get() : NULL);
#if 0
...
void LVDocView::drawPageHeader(LVDrawBuf* drawbuf, const lvRect& headerRc,
    ...
    if (!m_pageHeaderOverride.empty()) {
        ...
        int chapterEndPage = -1;
        LVTocItem* chapterTocItem = NULL;
        if (pageCount > 0) {
            if ((phi & (PGHDR_CHAPTER_PAGE_NUMBER | PGHDR_CHAPTER_PAGE_COUNT | PGHDR_CHAPTER_NAME)) && getToc()) {
                LVPtrVector<LVTocItem, false> tocItems;
                if (getFlatToc(tocItems)) {
                    bool toc0Based = false;
                    for (int j = 0; j < tocItems.length(); j++) {
                        if (tocItems[j]->getPage() == 0) { toc0Based = true; break; }
                    }
                    LVArray<int> sortedPages;
                    LVArray<LVTocItem*> tocForPage;
                    for (int i = 0; i < tocItems.length(); i++) {
                        LVTocItem* item = tocItems[i];
                        int p = item->getPage();
                        if (p < 0)
                            continue;
                        bool skipConvert = toc0Based;
                        LVTocItem* par = item->getParent();
                        if (par && par->getLevel() > 0 && par->getChildCount() > 0
                            && par->getChild(0) == item && par->getPage() < 0 && p > 0) {
                            int coverPage = p - 1;
                            bool dup = false;
                            for (int j = 0; j < sortedPages.length(); j++) {
                                if (sortedPages[j] == coverPage) { dup = true; break; }
                            }
                            if (!dup) {
                                sortedPages.add(coverPage);
                                tocForPage.add(par);
                            }
                            skipConvert = true;
                        }
                        int parPage = par ? par->getPage() : -1;
                        if (par && par->getLevel() > 0 && par->getChildCount() > 0
                            && par->getChild(0) == item && parPage >= 0 && parPage == p)
                            skipConvert = true;
                        if (!skipConvert && p > 0)
                            p = p - 1;
                        if (item->getChildCount() > 0 && p > 0) {
                            LVTocItem* fc = item->getChild(0);
                            if (fc && fc->getPage() == p)
                                p = p - 1;
                        }
                        bool dup = false;
                        for (int j = 0; j < sortedPages.length(); j++) {
                            if (sortedPages[j] == p) { dup = true; break; }
                        }
                        if (!dup) {
                            sortedPages.add(p);
                            tocForPage.add(item);
                    ...
                    int sortedCount = sortedPages.length();
                    for (int i = 0; i < sortedCount - 1; i++) {
                        for (int j = i + 1; j < sortedCount; j++) {
                            if (sortedPages[j] < sortedPages[i]) {
                                int tmp = sortedPages[i];
                                sortedPages[i] = sortedPages[j];
                                sortedPages[j] = tmp;
                                LVTocItem* t = tocForPage[i];
                                tocForPage[i] = tocForPage[j];
                                tocForPage[j] = t;
                            }
                        }
                    }
                    for (int i = sortedCount - 1; i >= 0; i--) {
                        if (sortedPages[i] <= pageIndex) {
                            chapterStartPage = sortedPages[i];
                            chapterEndPage = (i + 1 < sortedCount) ? sortedPages[i + 1] - 1 : pageCount - 1;
                            chapterTocItem = tocForPage[i];
                            break;
                        }
                    }
                }
            ...
            if (phi & PGHDR_PAGE_NUMBER)
                leftInfo += fmt::decimal(pageIndex + 1);
            if (phi & PGHDR_PAGE_COUNT) {
                if (!leftInfo.empty())
                    leftInfo += " / ";
                leftInfo += fmt::decimal(pageCount);
            }
            if ((phi & PGHDR_CHAPTER_PAGE_NUMBER) && chapterStartPage >= 0) {
                if (!leftInfo.empty())
                    leftInfo += "  ";
                leftInfo += fmt::decimal(pageIndex - chapterStartPage + 1);
            }
            if ((phi & PGHDR_CHAPTER_PAGE_COUNT) && chapterStartPage >= 0 && chapterEndPage >= chapterStartPage) {
                if (!leftInfo.empty())
                    leftInfo += " / ";
                leftInfo += fmt::decimal(chapterEndPage - chapterStartPage + 1);
            }
            ...
            if (batteryPercentNormalFont) {
                ...
                if (m_battery_charge_level >= 0 && m_battery_charge_level <= 100) {
                    ...
                    rightInfo << fmt::decimal(m_battery_charge_level) << "%]";
                } else {
                    if (m_battery_state == CR_BATTERY_STATE_CHARGING) {
                        rightInfo << Utf8ToUnicode("[ ↯ ]");
                    } else {
                        rightInfo << Utf8ToUnicode("[ ? ]");
                    }
                }
        ...
        if (phi & PGHDR_CLOCK) {
            if (!rightInfo.empty())
                rightInfo += "  ";
            rightInfo += getTimeString();
            m_last_clock = getTimeString();
        }

        int headerLeft = headerRc.left + m_headerMarginH;
        int headerRight = headerRc.right - m_headerMarginH;
        int leftW = leftInfo.empty() ? 0 : m_infoFont->getTextWidth(leftInfo.c_str(), leftInfo.length());
        int rightW = rightInfo.empty() ? 0 : m_infoFont->getTextWidth(rightInfo.c_str(), rightInfo.length());
        if (!leftInfo.empty())
            m_infoFont->DrawTextString(drawbuf, headerLeft, texty,
                                       leftInfo.c_str(), leftInfo.length(), U' ', NULL, false);
        if (!rightInfo.empty())
            m_infoFont->DrawTextString(drawbuf, headerRight - rightW, texty,
                                       rightInfo.c_str(), rightInfo.length(), U' ', NULL, false);
        info.left += leftW + (leftW > 0 ? 10 : 0);
        info.right -= rightW + (rightW > 0 ? 10 : 0);

        lString32 chapterName;
        if ((phi & PGHDR_CHAPTER_NAME) && chapterTocItem)
            chapterName = chapterTocItem->getName();
        int authorsw = 0;
        lString32 authors;
        if (phi & PGHDR_AUTHOR)
            authors = getAuthors();
        ...
        if ((phi & PGHDR_CHAPTER_NAME) && !chapterName.empty())
            text = chapterName;
        else if (text.empty() && !chapterName.empty())
            text = chapterName;
    }
    lvRect newcr = headerRc;
    newcr.left = info.left;
    newcr.right = info.right - 10;
    drawbuf->SetClipRect(&newcr);
    text = fitTextWidthWithEllipsis(text, m_infoFont, newcr.width());
    if (!text.empty()) {
        int centerW = m_infoFont->getTextWidth(text.c_str(), text.length());
        int centerX = headerRc.left + (headerRc.right - headerRc.left - centerW) / 2;
        if (centerX < newcr.left) centerX = newcr.left;
        if (centerX + centerW > newcr.right) centerX = newcr.right - centerW;
        m_infoFont->DrawTextString(drawbuf, centerX, texty, text.c_str(),
...
void LVDocView::drawPageTo(LVDrawBuf* drawbuf, LVRendPageInfo& page,
    ...
    if (m_doc) {
        if (page.flags & RN_PAGE_TYPE_COVER) {
            ...
            if (page.height)
                DrawDocument(*drawbuf, m_doc->getRootNode(), pageRect->left + m_pageMargins.left, clip.top, pageRect->width() - m_pageMargins.left - m_pageMargins.right, height, 0,
...
#define FOOTNOTE_MARGIN_REM 1 // as in lvpagesplitter.cpp
            ...
            if (footnotes_height > 0) {
                int h_avail = m_dy - getPageHeaderHeight() - m_pageMargins.top - m_pageMargins.bottom - height - footnote_margin;
            ...
            for (int fn = 0; fn < page.footnotes.length(); fn++) {
                ...
                drawbuf->SetClipRect(&clip);
                DrawDocument(*drawbuf, m_doc->getRootNode(), pageRect->left + m_pageMargins.left, fy + offset, pageRect->width() - m_pageMargins.left - m_pageMargins.right, fheight, 0,
...
bool LVDocView::docToWindowPoint(lvPoint& pt, bool isRectBottom, bool fitToPage) {
    ...
    if (getViewMode() == DVM_SCROLL) {
        ...
        if (fitToPage) {
            ...
            } else if (getVisiblePageCount() == 2 && page + 1 < m_pages.length() && pt.y >= m_pages[page + 1]->start + m_pages[page + 1]->height) {
...
void LVDocView::setRenderProps(int dx, int dy) {
    ...
    m_doc->setRenderProps(dx, dy, m_showCover, m_showCover ? dy + m_pageMargins.bottom * 4 : 0, m_font, m_def_interline_space, m_props);
...
lUInt32 LVDocView::getDocumentRenderingHash() const {
    if (m_doc) {
        ...
        return ((((lUInt32)m_doc->getDocumentRenderingHash()) * 31 + (lUInt32)m_doc->getFullHeight()) * 31 + (lUInt32)getPageCount());
    }
    return 0;
}
...
void LVDocView::createEmptyDocument() {
    ...
    // Footnotes mode: 0=hidden, 1=page_bottom, 2=inline, 3=inline_block, 4=quote
    ...
    m_doc->setDocFlag(DOC_FLAG_FOOTNOTES_INLINE_BLOCK, footnotesMode == 3);
    m_doc->setDocFlag(DOC_FLAG_FOOTNOTES_QUOTE, footnotesMode == 4);
...
int LVDocView::onSelectionCommand(int cmd, int param) {
    ...
    int y0 = GetPos();
    int h = m_pageRects[0].height() - m_pageMargins.top - m_pageMargins.bottom - getPageHeaderHeight();
    //int y1 = y0 + h;
...
static const char* def_style_macros[] = {
    "styles.def.align",
    "text-align: justify",
    "styles.def.text-indent",
    "text-indent: 1.2em",
    "styles.def.margin-top",
    "margin-top: 0em",
    "styles.def.margin-bottom",
    "margin-bottom: 0em",
    "styles.def.margin-left",
    "margin-left: 0em",
    "styles.def.margin-right",
    "margin-right: 0em",
    "styles.title.align",
    "text-align: center",
    "styles.title.text-indent",
    "text-indent: 0em",
    "styles.title.margin-top",
    "margin-top: 0.3em",
    "styles.title.margin-bottom",
    "margin-bottom: 0.3em",
    "styles.title.margin-left",
    "margin-left: 0em",
    "styles.title.margin-right",
    "margin-right: 0em",
    "styles.title.font-size",
    "font-size: 110%",
    "styles.title.font-weight",
    "font-weight: bolder",
    "styles.subtitle.align",
    "text-align: center",
    "styles.subtitle.text-indent",
    "text-indent: 0em",
    "styles.subtitle.margin-top",
    "margin-top: 0.2em",
    "styles.subtitle.margin-bottom",
    "margin-bottom: 0.2em",
    "styles.subtitle.font-style",
    "font-style: italic",
    "styles.cite.align",
    "text-align: justify",
    "styles.cite.text-indent",
    "text-indent: 1.2em",
    "styles.cite.margin-top",
    "margin-top: 0.3em",
    "styles.cite.margin-bottom",
    "margin-bottom: 0.3em",
    "styles.cite.margin-left",
    "margin-left: 1em",
    "styles.cite.margin-right",
    "margin-right: 1em",
    "styles.cite.font-style",
    "font-style: italic",
    "styles.epigraph.align",
    "text-align: left",
    "styles.epigraph.text-indent",
    "text-indent: 1.2em",
    "styles.epigraph.margin-top",
    "margin-top: 0.3em",
    "styles.epigraph.margin-bottom",
    "margin-bottom: 0.3em",
    "styles.epigraph.margin-left",
    "margin-left: 15%",
    "styles.epigraph.margin-right",
    "margin-right: 1em",
    "styles.epigraph.font-style",
    "font-style: italic",
    "styles.pre.align",
    "text-align: left",
    "styles.pre.text-indent",
    "text-indent: 0em",
    "styles.pre.margin-top",
    "margin-top: 0em",
    "styles.pre.margin-bottom",
    "margin-bottom: 0em",
    "styles.pre.margin-left",
    "margin-left: 0em",
    ...
    "text-indent: 0em",
    "styles.poem.margin-top",
    "margin-top: 0.3em",
    "styles.poem.margin-bottom",
    "margin-bottom: 0.3em",
    "styles.poem.margin-left",
    "margin-left: 15%",
    "styles.poem.margin-right",
    "margin-right: 1em",
    "styles.poem.font-style",
    "font-style: italic",
    "styles.text-author.font-style",
    "font-style: italic",
    "styles.text-author.font-weight",
    "font-weight: bolder",
    "styles.text-author.margin-left",
    "margin-left: 1em",
    "styles.text-author.font-style",
    "font-style: italic",
    "styles.text-author.font-weight",
    ...
    "styles.text-author.margin-left",
    "margin-left: 1em",
    "styles.link.text-decoration",
    "text-decoration: underline",
    "styles.footnote-link.font-size",
    "font-size: 70%",
    "styles.footnote-link.vertical-align",
    "vertical-align: super",
    "styles.footnote",
    "font-size: 70%",
    "styles.footnote-title",
    "font-size: 110%",
    "styles.annotation.font-size",
    "font-size: 90%",
    "styles.annotation.margin-left",
    "margin-left: 1em",
    "styles.annotation.margin-right",
    "margin-right: 1em",
    "styles.annotation.font-style",
    "font-style: italic",
    "styles.annotation.align",
    "text-align: justify",
    "styles.annotation.text-indent",
    "text-indent: 1.2em",
    NULL,
    NULL,
};
...
void LVDocView::propsUpdateDefaults(CRPropRef props) {
    ...
    props->limitValueMinMax(PROP_FOOTNOTES_MODE, 0, 4, 1); // 0=hidden, 1=page_bottom (default), 2=inline, 3=inline_block, 4=quote
    ...
    props->setBoolDef(PROP_SHOW_CHAPTER_NAME, false);
    props->setBoolDef(PROP_SHOW_CHAPTER_PAGE_NUMBER, false);
    props->setBoolDef(PROP_SHOW_CHAPTER_PAGE_COUNT, false);
...
void LVDocView::setStatusMode(int pos, bool showClock, bool showTitle,
                              bool showBattery, bool showBatteryPercent, bool showNavbar, bool showChapterMarks, bool showPosPercent, bool showPageNumber, bool showPageCount, bool showChapterName, bool showChapterPageNumber, bool showChapterPageCount) {
    ...
    if (showPageCount)
        phi |= PGHDR_PAGE_COUNT;
    if (showChapterName)
        phi |= PGHDR_CHAPTER_NAME;
    if (showChapterPageNumber)
        phi |= PGHDR_CHAPTER_PAGE_NUMBER;
    if (showChapterPageCount)
        phi |= PGHDR_CHAPTER_PAGE_COUNT;
...
CRPropRef LVDocView::propsApply(CRPropRef props) {
    ...
    for (int i = 0; i < props->getCount(); i++) {
        ...
        if (name == PROP_FONT_ANTIALIASING) {
        ...
        } else if (name == PROP_STATUS_LINE || name == PROP_SHOW_TIME || name == PROP_SHOW_TITLE || name == PROP_SHOW_BATTERY || name == PROP_SHOW_BATTERY_PERCENT || name == PROP_STATUS_NAVBAR || name == PROP_STATUS_CHAPTER_MARKS || name == PROP_SHOW_POS_PERCENT || name == PROP_SHOW_PAGE_COUNT || name == PROP_SHOW_PAGE_NUMBER || name == PROP_SHOW_CHAPTER_NAME || name == PROP_SHOW_CHAPTER_PAGE_NUMBER || name == PROP_SHOW_CHAPTER_PAGE_COUNT) {
            m_props->setString(name.c_str(), value);
            setStatusMode(m_props->getIntDef(PROP_STATUS_LINE, 1),
                          ...
                          m_props->getBoolDef(PROP_SHOW_CHAPTER_NAME, false),
                          m_props->getBoolDef(PROP_SHOW_CHAPTER_PAGE_NUMBER, false),
                          m_props->getBoolDef(PROP_SHOW_CHAPTER_PAGE_COUNT, false));
        ...
        } else if (name == PROP_HYPHENATION_DICT || name == PROP_HYPHENATION_LEFT_HYPHEN_MIN || name == PROP_HYPHENATION_RIGHT_HYPHEN_MIN || name == PROP_HYPHENATION_TRUST_SOFT_HYPHENS || name == PROP_TEXTLANG_MAIN_LANG || name == PROP_TEXTLANG_EMBEDDED_LANGS_ENABLED || name == PROP_TEXTLANG_HYPHENATION_ENABLED || name == PROP_TEXTLANG_HYPH_SOFT_HYPHENS_ONLY || name == PROP_TEXTLANG_HYPH_FORCE_ALGORITHMIC) {
            ...
            int mode = props->getIntDef(PROP_FOOTNOTES_MODE, 1); // 0=hidden, 1=page_bottom, 2=inline, 3=inline_block, 4=quote
            if (m_doc) { // not when noDefaultDocument=true
                ...
                // mode 3 = inline_block: enable footnotes as inline blocks (starting on next line)
                // mode 4 = quote: append note blocks at paragraph end
                getDocument()->setDocFlag(DOC_FLAG_ENABLE_FOOTNOTES, mode > 0);
                getDocument()->setDocFlag(DOC_FLAG_FOOTNOTES_INLINE, mode == 2);
                getDocument()->setDocFlag(DOC_FLAG_FOOTNOTES_INLINE_BLOCK, mode == 3);
                getDocument()->setDocFlag(DOC_FLAG_FOOTNOTES_QUOTE, mode == 4);
