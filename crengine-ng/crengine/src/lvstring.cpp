...
lString32& lString32::trimNonAlpha() {
    ...
    for (firstns = 0; firstns < pchunk->len &&
                      !isAlNum(pchunk->buf._32[firstns]);
         ++firstns)
        ;
...
lString32& lString32::trim() {
    ...
    int firstns;
    for (firstns = 0; firstns < pchunk->len &&
                      (pchunk->buf._32[firstns] == ' ' || pchunk->buf._32[firstns] == '\t');
         ++firstns)
        ;
    if (firstns >= pchunk->len) {
        clear();
    ...
    int lastns;
    for (lastns = pchunk->len - 1; lastns > 0 &&
                                   (pchunk->buf._32[lastns] == ' ' || pchunk->buf._32[lastns] == '\t');
         --lastns)
        ;
    int newlen = lastns - firstns + 1;
    if (newlen == pchunk->len)
...
lString16& lString16::trimNonAlpha() {
    int firstns;
    for (firstns = 0; firstns < pchunk->len &&
                      !isAlNum(pchunk->buf._16[firstns]);
         ++firstns)
        ;
    if (firstns >= pchunk->len) {
        clear();
...
lString16& lString16::trim() {
    ...
    int firstns;
    for (firstns = 0; firstns < pchunk->len &&
                      (pchunk->buf._16[firstns] == ' ' || pchunk->buf._16[firstns] == '\t');
         ++firstns)
        ;
    if (firstns >= pchunk->len) {
        clear();
    ...
    int lastns;
    for (lastns = pchunk->len - 1; lastns > 0 &&
                                   (pchunk->buf._16[lastns] == ' ' || pchunk->buf._16[lastns] == '\t');
         --lastns)
        ;
    int newlen = lastns - firstns + 1;
    if (newlen == pchunk->len)
...
int lString32::pos(const lChar8* subStr) const {
    ...
    for (int i = 0; i <= dl; i++) {
        int flg = 1;
        for (int j = 0; j < l; j++)
            if (pchunk->buf._32[i + j] != (value_type)subStr[j]) {
                flg = 0;
                break;
            }
        if (flg)
...
static void DecodeUtf8(const char* s, lChar32* p, int len) {
    ...
    while (p < endp) {
        ...
        if ((ch & 0x80) == 0) {
            ...
            *p++ = ((ch & 0x1F) << 6) | CONT_BYTE(0, 0);
            s++;
        } else if ((ch & 0xF0) == 0xE0) {
            *p++ = ((ch & 0x0F) << 12) | CONT_BYTE(0, 6) | CONT_BYTE(1, 0);
            s += 2;
        } else if ((ch & 0xF8) == 0xF0) {
            *p++ = ((ch & 0x07) << 18) | CONT_BYTE(0, 12) | CONT_BYTE(1, 6) | CONT_BYTE(2, 0);
            s += 3;
        } else {
    ...
    while (p < endp) {
        ...
        if ((ch & 0x80) == 0) {
            ...
            *p++ = (char)ch;
            ...
            matched = true;
            *p++ = ((ch & 0x1F) << 6) | CONT_BYTE(1, 0);
            s += 2;
        } else if ((ch & 0xF0) == 0xE0) {
            matched = true;
            *p++ = ((ch & 0x0F) << 12) | CONT_BYTE(1, 6) | CONT_BYTE(2, 0);
            s += 3;
            ...
            // Mostly unused
            matched = true;
            *p++ = ((ch & 0x07) << 18) | CONT_BYTE(1, 12) | CONT_BYTE(2, 6) | CONT_BYTE(3, 0);
            s += 4;
        } else {
            // Invalid first byte in UTF-8 sequence
            // Pass with mask 0x7F, to resolve exception around env->NewStringUTF()
...
inline lUInt16 getCharProp(lChar32 ch) {
    ...
    else if (ch >= UNICODE_CJK_IDEOGRAPHS_BEGIN && ch <= UNICODE_CJK_IDEOGRAPHS_END && (ch <= UNICODE_CJK_PUNCTUATION_HALF_AND_FULL_WIDTH_BEGIN || ch >= UNICODE_CJK_PUNCTUATION_HALF_AND_FULL_WIDTH_END))
        return CH_PROP_CJK;
...
bool lStr_isWordSeparator(lChar32 ch) {
    ...
    if (ch >= 0x61 && ch <= 0x7A)
        return false; // lowercase ascii letters
    if (ch >= 0x41 && ch <= 0x5A)
        return false; // uppercase ascii letters
    if (ch >= 0x30 && ch <= 0x39)
        return false; // digits
    if (ch == 0xAD)
        return false; // soft-hyphen, considered now as part of word
    ...
    // add here :
    if (ch == 0xD7)
        return true; // multiplication sign
    if (ch == 0xF7)
        return true; // division sign
    // this one includes em-dash & friends, and other quotation marks
    if (ch >= UNICODE_GENERAL_PUNCTUATION_BEGIN && ch <= UNICODE_GENERAL_PUNCTUATION_END)
        return true;
    // CJK puncutation
    ...
    if (ch == 0x180E)
        return true; // MONGOLIAN VOWEL SEPARATOR
    if (ch == 0xFEFF)
        return true; // ZERO WIDTH NO-BREAK SPACE
