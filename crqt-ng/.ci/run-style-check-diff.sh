...
# For now, clang-format is only used for the "src" directory, when
# another directory is formatted with clang-format, add it to this variable.
ALLOWED_CLANG_FORMAT_DIRS="src"
...
if [ "x${CI_PROJECT_NAMESPACE}" != "x" -a "${CI_PROJECT_NAMESPACE}" != "coolreader-ng" ]; then
    ...
    git remote add upstream https://gitlab.com/coolreader-ng/crqt-ng.git
    ORIGIN="upstream"
else
