...
template <typename T, int RESIZE_MULT, int RESIZE_ADD>
class CompactArray
{
    struct Array
    {
...
public:
    ...
    LVRendPageInfo(int pageStart, lUInt16 pageHeight, int pageIndex)
            ...
            , index(pageIndex)
            , height(pageHeight)
            , flags(RN_PAGE_TYPE_NORMAL)
            , flow(0) { }
    LVRendPageInfo(lUInt16 coverHeight)
            : start(0)
            , index(0)
            , height(coverHeight)
            , flags(RN_PAGE_TYPE_COVER)
            , flow(0) { }
    LVRendPageInfo()
            : start(0)
            , index(0)
...
public:
    ...
    int getSplitBefore() const {
        return (flags >> RN_SPLIT_BEFORE) & 7;
    }
    int getSplitAfter() const {
        return (flags >> RN_SPLIT_AFTER) & 7;
    }
    /*
...
public:
    ...
    CompactArray<LVRendLineInfo*, 2, 4>& getLines() {
        return lines;
    }
    bool empty() {
        return lines.empty();
...
public:
    void setCallback(LVDocViewCallback* cb, int _totalFinalBlocks) {
        callback = cb;
        totalFinalBlocks = _totalFinalBlocks;
    ...
    lString32Collection* getLinkIds() {
        return &link_ids;
    }
