...
/// same as getFont but use sizeOverride when > 0 (clamped 8..340); used e.g. for quote block smaller text
...
void renderFinalBlock(ldomNode* node, LFormattedText* txform, RenderRectAccessor* fmt, lUInt32& flags,
                      int indent, int line_h, TextLangCfg* lang_cfg = NULL, int valign_dy = 0,
                      bool* is_link_start = NULL, lString32Collection* quote_footnotes = NULL);
