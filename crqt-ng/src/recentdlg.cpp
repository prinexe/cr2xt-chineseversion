...
RecentBooksDlg::RecentBooksDlg(QWidget* parent, CR3View* docView)
    ...
    m_ui->tableWidget->setHorizontalHeaderLabels(QStringList()
                                                 << tr("#") << tr("Author") << tr("Title") << tr("Filename"));
    ...
    for (int i = firstItem; i < files.length(); i++) {
        ...
        if (title.empty())
            ...
            title << " - " << series;
        int index = 0;
        m_ui->tableWidget->setItem(i - firstItem, index++,
                                   new QTableWidgetItem(cr2qt(lString32::itoa(i - firstItem + 1))));
...
void RecentBooksDlg::on_actionClearAll_triggered() {
    //
    if (QMessageBox::question(this, tr("Remove all history items"),
                              tr("Do you really want to remove all history records?"),
