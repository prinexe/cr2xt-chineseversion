/***************************************************************************
 *   crengine-ng                                                           *
 *   Copyright (C) 2007-2015,2019 Vadim Lopatin <coolreader.org@gmail.com> *
 ...
 *   Copyright (C) 2011,2013,2015,2019 Konstantin Potapov <pkbo@users.sourceforge.net>
...
/**
 * \file lvrend.cpp
 * \brief XML DOM tree rendering tools
 */
...
public:
    FILE* f;
    simpleLogFile(const char* fname) {
        f = fopen(fname, "wt");
    }
    ~simpleLogFile() {
        if (f)
    ...
    simpleLogFile& operator<<(const lString32& str) {
...
{
public:
    simpleLogFile& operator<<(const char*) {
        return *this;
    }
    simpleLogFile& operator<<(int) {
        return *this;
    }
    simpleLogFile& operator<<(const lChar32*) {
        return *this;
    }
    simpleLogFile& operator<<(const lString32&) {
        return *this;
    }
};
...
void collapse_border(css_style_ref_t& target_style, int& current_target_size,
    ...
    if (neighbour_node) {
        ...
        if (neighbour_size > current_target_size ||
            ...
            switch (border_id) {
                case 0:
                    target_style->border_style_top = neighbour_style->border_style_top;
                    break;
                case 1:
                    target_style->border_style_right = neighbour_style->border_style_right;
                    break;
                case 2:
                    target_style->border_style_bottom = neighbour_style->border_style_bottom;
                    break;
                case 3:
                    target_style->border_style_left = neighbour_style->border_style_left;
                    break;
            }
...
public:
    ...
    void PlaceCells() {
        ...
        if (border_collapse) {
            ...
            table_paddings_width = lengthToPx(elem, table_style->padding[0], table_width) + lengthToPx(elem, table_style->padding[1], table_width);
        ...
        if (distribute_restw) {
            ...
            if (restw != 0 && dist_nb_cols > 0) {
                ...
                for (i = 0; i < cols.length(); i++) {
                    if ((!dist_all_non_empty_cols && cols[i]->min_width > 0) || (dist_all_non_empty_cols && cols[i]->min_width != 0)) {
                        cols[i]->width += a;
                        if (b > 0) {
                            cols[i]->width++;
                            b--;
            ...
            for (j = 0; j < rows[i]->cells.length(); j++) {
                ...
                if (i == y) { // upper left corner of cell
                    ...
                    if (cell->elem->getRendMethod() == erm_final) {
                        ...
                        // When DOC_FLAG_FOOTNOTES_INLINE / INLINE_BLOCK / QUOTE is set,
                        // footnotes are not shown in page-bottom mode.
                        bool collectFootnoteLinks = elem->getDocument()->getDocFlag(DOC_FLAG_ENABLE_FOOTNOTES) &&
                                                    ...
                                                    !elem->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE);
                                    ...
                                    if (line->words[w].flags & LTEXT_WORD_IS_LINK_START) {
                                        ...
                                        if (src && src->object) {
                                            ...
                                            if (parent && parent->hasAttribute(LXML_NS_ANY, attr_href)) {
                                                ...
                                                lString32 id;
                                                int sharpPos = href.pos("#");
                                                if (sharpPos >= 0 && sharpPos + 1 < (int)href.length())
                                                    id = href.substr(sharpPos + 1);
                                                if (!id.empty()) {
                                                    if (is_single_column)
                                                        row->single_col_context->addLink(id, link_insert_pos);
                                                    else
                                                        row->links.add(id);
                                                }
                                            }
...
bool isSameFontStyle(css_style_rec_t* style1, css_style_rec_t* style2) {
    return (style1->font_family == style2->font_family) && (style1->font_size == style2->font_size) && (style1->font_style == style2->font_style) && (style1->font_name == style2->font_name) && (style1->font_weight == style2->font_weight);
}
...
LVFontRef getFontWithSizeOverride(ldomNode* node, const css_style_rec_t* style, int documentId, int sizeOverride) {
    int sz;
    if (sizeOverride > 0) {
        sz = sizeOverride;
        if (sz < 8) sz = 8;
        ...
        if (style->font_size.type == css_val_em || style->font_size.type == css_val_ex ||
            style->font_size.type == css_val_ch || style->font_size.type == css_val_percent) {
            CRLog::warn("CRE WARNING: getFontWithSizeOverride: %d of unit %d\n", style->font_size.value >> 8, style->font_size.type);
            sz = style->font_size.value >> 8;
        } else {
            sz = lengthToPx(node, style->font_size, 0, 0);
        }
        if (sz < 8) sz = 8;
    ...
    fw += (rend_font_base_weight - 400);
    if (fw < 1) fw = 1;
    else if (fw > 999) fw = 999;
    lString8 font_name = style->font_name;
    lString8 genericFontFamilyFace = getGenericFontFamilyFace(style->font_family);
    if (font_name.empty())
        font_name = genericFontFamilyFace;
    LVFontRef fnt = fontMan->GetFont(
            sz, fw, style->font_style == css_fs_italic, style->font_family, font_name,
            style->font_features.value, documentId, true);
    if (fnt.isNull() || (font_name != genericFontFamilyFace && -1 == font_name.pos(fnt->getTypeFace()))) {
        fnt = fontMan->GetFont(
                sz, fw, style->font_style == css_fs_italic, style->font_family, genericFontFamilyFace,
                style->font_features.value, documentId, true);
    }
    return fnt;
}
...
bool getStyledImageSize(ldomNode* enode, int& img_width, int& img_height, int container_width, int container_height, bool enforce_page_constraints) {
    ...
    bool is_el_image;
    if (enode->getNodeId() == el_img) { // HTML/EPUB <img>
        is_el_image = false;
    } else if (enode->getNodeId() == el_image) { // FB2/SVG <image>
        is_el_image = true;
    } else {
        return false;
    }
    ...
    if (is_el_image && width < 0 && height < 0) {
        // Try to handle this really common SVG wrapping generated by some Calibre plugin:
        //   <body>
        //     <div>
...
// If the first non-whitespace child of enode is an <a> element with an id attribute,
// return that id. Otherwise return empty string.
// Used to identify EPUB footnote bodies of the form:
//   <p class="fncontent"><a id="fn1" href="...">marker</a>text</p>
// and distinguish them from body paragraphs where <a id> appears mid-text:
//   <p class="bodycontent">text<a id="back" href="..."><sup>(2)</sup></a>text</p>
static lString32 getFirstChildAnchorId(ldomNode* enode) {
    int cnt = enode->getChildCount();
    for (int i = 0; i < cnt; i++) {
        ldomNode* child = enode->getChildNode(i);
        if (child->isText()) {
            if (child->getText().trim().empty()) continue; // skip whitespace-only text
            return lString32::empty_str; // non-whitespace text before any element
        }
        if (child->isElement()) {
            if (child->getNodeId() == el_a && child->hasAttribute(LXML_NS_ANY, attr_id))
                return child->getAttributeValue(LXML_NS_ANY, attr_id);
            return lString32::empty_str; // first element is not <a id>
        }
    }
    return lString32::empty_str;
}

static inline bool isCjkQuoteChar(lChar32 ch) {
...
static inline bool isQuoteWhitespace(lChar32 ch) {
    if (ch == U' ' || ch == U'\t' || ch == U'\r' || ch == U'\n')
        return true;
    // Common Unicode spaces often found in OCR/ebook sources.
    if (ch == 0x0085 || ch == 0x00A0 || ch == 0x1680 || ch == 0x2028 || ch == 0x2029 || ch == 0x202F || ch == 0x205F || ch == 0x3000)
        return true;
...
static inline bool isQuoteIgnorableChar(lChar32 ch) {
    // Zero-width / formatting chars frequently found in OCR or EPUB text runs.
...
// Normalize quote footnote text:
// - collapse all whitespace runs (including line breaks/unicode spaces) into one plain space
// - optionally remove spaces inserted between two CJK chars
static lString32 normalizeQuoteFootnoteText(const lString32& src) {
    lString32 out;
    out.reserve(src.length());
    int i = 0;
    while (i < src.length()) {
        lChar32 ch = src[i];
        if (isQuoteIgnorableChar(ch)) {
            i++;
            continue;
        }
        if (isQuoteWhitespace(ch)) {
            lChar32 prev = out.empty() ? 0 : out.lastChar();
            lChar32 next = 0;
            int j = i + 1;
            while (j < src.length() && (isQuoteWhitespace(src[j]) || isQuoteIgnorableChar(src[j])))
                j++;
            if (j < src.length())
                next = src[j];
            // Drop artificial spacing between adjacent CJK chars.
            if (!(isCjkQuoteChar(prev) && isCjkQuoteChar(next))) {
                if (!out.empty() && out.lastChar() != U' ')
                    out << U' ';
            }
            i = j;
            continue;
...
static bool isQuoteDebugEnabled() {
    static int enabled = -1;
    if (enabled == -1) {
        const char* env = getenv("CR3_QUOTE_DEBUG");
        enabled = (env && *env && strcmp(env, "0") != 0) ? 1 : 0;
    }
    return enabled == 1;
}

static void quoteDebugLog(const char* fmt, ...) {
    if (!isQuoteDebugEnabled())
        return;
    static FILE* fp = NULL;
    if (!fp) {
        const char* path = getenv("CR3_QUOTE_DEBUG_FILE");
        if (!path || !*path)
            path = "/tmp/cr3_quote_debug.log";
        fp = fopen(path, "a");
        if (!fp)
            return;
    }
...
static lString8 quoteDebugPreview(const lString32& src, int maxChars = 160) {
    lString32 s = src;
    s.trim();
...
// Get the first non-empty paragraph from a text where '\n' separates blocks.
// This helps keep quote notes scoped to their primary footnote paragraph and
// avoid pulling following body paragraphs when containers are broad.
static lString32 getFirstParagraphText(const lString32& src) {
    int i = 0;
    while (i < src.length()) {
        while (i < src.length() && (src[i] == U'\r' || src[i] == U'\n'))
            i++;
        if (i >= src.length())
            break;
        int start = i;
        while (i < src.length() && src[i] != U'\r' && src[i] != U'\n')
            i++;
        lString32 para = src.substr(start, i - start);
        para.trim();
        if (!para.empty())
            return para;
    }
...
static bool hasNoteLikeIdOrClass(ldomNode* node) {
    if (!node || node->isNull() || !node->isElement())
        return false;
    lString32 id;
    lString32 cls;
    if (node->hasAttribute(LXML_NS_ANY, attr_id))
        id = node->getAttributeValue(LXML_NS_ANY, attr_id);
    if (node->hasAttribute(LXML_NS_ANY, attr_class))
        cls = node->getAttributeValue(LXML_NS_ANY, attr_class);
    id.lowercase();
    cls.lowercase();
    // Keep this conservative: broad "ref" matching causes false positives
    // (e.g. classes like "preface", "reference-list", etc.).
    return id.pos("note") >= 0 || id.pos("foot") >= 0 || id.pos("fn") >= 0 ||
           id.pos("cite") >= 0 || id.pos("annot") >= 0 ||
           cls.pos("note") >= 0 || cls.pos("foot") >= 0 || cls.pos("fn") >= 0 ||
           cls.pos("cite") >= 0 || cls.pos("annot") >= 0;
}

static bool isBackLinkLikeAttr(const lString32& s) {
    if (s.empty())
        return false;
    lString32 v = s;
    v.lowercase();
    return v.pos("backlink") >= 0 || v.pos("back-ref") >= 0 || v.pos("backref") >= 0 ||
           v.pos("return") >= 0 || v.pos("noteback") >= 0;
}

static bool isLikelyFootnoteTargetId(const lString32& id) {
    if (id.empty())
        return false;
    lString32 v = id;
    v.lowercase();
    return v.pos("note") >= 0 || v.pos("foot") >= 0 || v.pos("fn") >= 0 || v.pos("annot") >= 0;
}

static bool isLikelyBackReferenceLink(ldomNode* linkNode, const lString32& targetId) {
    if (!linkNode || linkNode->isNull() || linkNode->getNodeId() != el_a)
        return false;
    // EPUB semantics (doc-backlink) and common class/id naming.
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_role) &&
        isBackLinkLikeAttr(linkNode->getAttributeValue(LXML_NS_ANY, attr_role)))
        return true;
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_type) &&
        isBackLinkLikeAttr(linkNode->getAttributeValue(LXML_NS_ANY, attr_type)))
        return true;
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_class) &&
        isBackLinkLikeAttr(linkNode->getAttributeValue(LXML_NS_ANY, attr_class)))
        return true;
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_id) &&
        isBackLinkLikeAttr(linkNode->getAttributeValue(LXML_NS_ANY, attr_id)))
        return true;
    // Common target ids for "return to text" anchors.
    lString32 tid = targetId;
    tid.lowercase();
    if (tid.pos("fnref") >= 0 || tid.pos("noteref") >= 0 || tid.pos("back") >= 0)
        return true;
    return false;
}

static bool isLikelyFootnoteRefLink(ldomNode* linkNode) {
    if (!linkNode || linkNode->isNull() || linkNode->getNodeId() != el_a)
        return false;
    // Semantic attributes if present.
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_type)) {
        lString32 t = linkNode->getAttributeValue(LXML_NS_ANY, attr_type);
        t.lowercase();
        if (t.pos("note") >= 0 || t.pos("noteref") >= 0 || t.pos("foot") >= 0)
            return true;
    }
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_role)) {
        lString32 r = linkNode->getAttributeValue(LXML_NS_ANY, attr_role);
        r.lowercase();
        if (r.pos("note") >= 0 || r.pos("noteref") >= 0 || r.pos("foot") >= 0)
            return true;
    }
    if (linkNode->hasAttribute(LXML_NS_ANY, attr_class)) {
        lString32 c = linkNode->getAttributeValue(LXML_NS_ANY, attr_class);
        c.lowercase();
        if (c.pos("note") >= 0 || c.pos("noteref") >= 0 || c.pos("foot") >= 0 || c.pos("fn") >= 0 ||
            c.pos("xref") >= 0 || c.pos("cite") >= 0)
            return true;
    }
    // Heuristic marker text: short and mostly marker-like chars.
    lString32 txt = linkNode->getText(0, 32);
    txt.trim();
    if (txt.empty() || txt.length() > 16)
        return false;
    // Bracket-wrapped text is a strong footnote marker signal: [1], (2), etc.
    if (txt.length() >= 3) {
        lChar32 first = txt[0];
        lChar32 last = txt[txt.length() - 1];
    ...
    for (int i = 0; i < txt.length(); i++) {
        ...
        if ((ch >= '0' && ch <= '9') || ch == '(' || ch == ')' || ch == '[' || ch == ']' ||
            ch == '*' || ch == '#' || ch == '.' || ch == 0x2020 || ch == 0x2021) {
            markerChars++;
        }
    }
    if (markerChars == 0)
        return false;
    // Without brackets, require <sup> context to distinguish footnote refs
    // from TOC/navigation links (which also have short numeric text).
    // Check <sup> as ancestor: <sup><a>4</a></sup>
    ldomNode* parent = linkNode->getParentNode();
    for (int i = 0; i < 3 && parent && !parent->isNull(); i++) {
        if (parent->isElement() && parent->getNodeId() == el_sup)
            return true;
        parent = parent->getParentNode();
    }
    // Check <sup> as direct child: <a><sup>4</sup></a>
    for (int i = 0; i < linkNode->getChildCount(); i++) {
        ldomNode* child = linkNode->getChildNode(i);
        if (child && child->isElement() && child->getNodeId() == el_sup)
            return true;
    }
    return false;
}

// Check if a link node should be excluded from quote footnote processing.
// Returns true for links inside footnote body sections (back-references)
// or navigation sections (TOC links), which should not generate quote annotations.
static bool isQuoteProcessingExcluded(ldomNode* linkNode) {
    if (!linkNode || linkNode->isNull())
        return false;
    const int maxAncestors = 15;
    ldomNode* cur = linkNode->getParentNode();
    for (int i = 0; i < maxAncestors && cur && !cur->isNull(); i++) {
        if (!cur->isElement()) {
            cur = cur->getParentNode();
            continue;
        }
        lUInt16 nid = cur->getNodeId();
        if (nid == el_nav)
            return true;
        css_style_ref_t st = cur->getStyle();
        if (!st.isNull() && STYLE_HAS_CR_HINT(st, FOOTNOTE_INPAGE))
            return true;
        if (hasNoteLikeIdOrClass(cur))
            return true;
        if (nid == el_body)
            break;
        cur = cur->getParentNode();
...
// Resolve the best footnote text container for a link target id.
// We keep this strict to avoid capturing surrounding body text:
// - Prefer ancestors marked with FOOTNOTE_INPAGE and whose resolved footnote id
//   matches the target id.
// - In quote mode only, allow a limited fallback to a block ancestor whose first
//   child anchor id matches the target id (common fncontent pattern).
static ldomNode* resolveFootnoteContainer(ldomNode* noteBody, const lString32& targetId, bool allowQuoteFallback) {
    if (!noteBody || targetId.empty())
        return NULL;
    // Quick pre-check: FOOTNOTE_INPAGE is typically within 3 ancestors of the target.
    // For non-footnote targets (chapter headings, TOC targets, etc.), avoid walking the
    // entire ancestor chain by checking a few levels first.
    if (!allowQuoteFallback) {
        bool hintFound = false;
        ldomNode* qc = noteBody;
        for (int i = 0; i < 4 && qc && !qc->isNull(); i++) {
            if (qc->isElement()) {
                css_style_ref_t st = qc->getStyle();
                if (!st.isNull() && STYLE_HAS_CR_HINT(st, FOOTNOTE_INPAGE)) {
                    hintFound = true;
                    break;
                }
    ...
    ldomNode* cur = noteBody;
    while (cur && !cur->isNull()) {
        if (cur->isElement()) {
            css_style_ref_t st = cur->getStyle();
            if (!st.isNull() && STYLE_HAS_CR_HINT(st, FOOTNOTE_INPAGE)) {
                lString32 footnoteId;
                if (cur->hasAttribute(LXML_NS_ANY, attr_id))
                    footnoteId = cur->getAttributeValue(LXML_NS_ANY, attr_id);
                if (footnoteId.empty())
                    footnoteId = getFirstChildAnchorId(cur);
                if (footnoteId == targetId)
                    return cur;
            }
        }
        cur = cur->getParentNode();
    ...
    if (allowQuoteFallback) {
        cur = noteBody->getParentNode();
        while (cur && !cur->isNull()) {
            if (cur->isElement()) {
                css_style_ref_t st = cur->getStyle();
                if (!st.isNull() && st->display == css_d_block) {
                    if (hasNoteLikeIdOrClass(cur) && getFirstChildAnchorId(cur) == targetId)
                        return cur;
                }
            }
            cur = cur->getParentNode();
        }
        // Conservative last-resort: some books use plain ids (e.g. "1_0", "1_1") for
        // footnote blocks without note-like class/id. Only when allowQuoteFallback is
        // true (link already deemed a footnote ref), accept first block ancestor that
        // is not a structural element and has limited text (real footnotes are short).
        const int maxLevels = 5;
        const int maxNoteChars = 600;
        cur = noteBody;
        for (int level = 0; cur && !cur->isNull() && level <= maxLevels; level++) {
            if (cur->isElement()) {
                lUInt16 nid = cur->getNodeId();
                if (nid == el_section || nid == el_body)
                    break;
                if (nid == el_h1 || nid == el_h2 || nid == el_h3 ||
                    nid == el_h4 || nid == el_h5 || nid == el_h6)
                    { cur = cur->getParentNode(); continue; }
                css_style_ref_t st = cur->getStyle();
                if (!st.isNull() && st->display == css_d_block) {
                    lString32 blockText = cur->getText(0, maxNoteChars + 1);
                    if (blockText.length() < maxNoteChars)
                        return cur;
                }
            }
            cur = cur->getParentNode();
...
void renderFinalBlock(ldomNode* enode, LFormattedText* txform, RenderRectAccessor* fmt, lUInt32& baseflags,
    ...
    if (enode->isElement()) {
        ...
        bool isQuoteFootnotes = enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE);
        if (enode->getNodeId() == el_a && (isInlineFootnotes || isInlineBlockFootnotes || isQuoteFootnotes)) {
            ...
            if (sharpPos >= 0 && sharpPos + 1 < (int)href.length())
                id = href.substr(sharpPos + 1);
            if (!id.empty()) {
                ldomNode* noteBody = enode->getDocument()->getElementById(id.c_str());
                if (noteBody) {
                    bool _quoteDbg = isQuoteFootnotes && isQuoteDebugEnabled();

                    bool _srcExcluded = isQuoteFootnotes && isQuoteProcessingExcluded(enode);
                    bool _isBackRef = isQuoteFootnotes && isLikelyBackReferenceLink(enode, id);
                    bool quoteExcluded = _srcExcluded || _isBackRef;
                    bool _isRefLink = !quoteExcluded && isQuoteFootnotes && isLikelyFootnoteRefLink(enode);
                    bool _isTargetId = !quoteExcluded && isQuoteFootnotes && isLikelyFootnoteTargetId(id);
                    bool allowQuoteFallback = _isRefLink || _isTargetId;
                    ldomNode* footnoteContainer = quoteExcluded ? NULL : resolveFootnoteContainer(noteBody, id, allowQuoteFallback);
                    bool isFootnoteTarget = footnoteContainer != NULL;

                    if (isQuoteFootnotes) {
                        lString8 linkText8 = UnicodeToUtf8(enode->getText(0, 32));
                        lString8 targetId8 = UnicodeToUtf8(id);
                        lString8 hrefAttr8 = UnicodeToUtf8(enode->getAttributeValue(LXML_NS_ANY, attr_href));
                        CRLog::error("[QUOTE-DIAG] href=\"%s\" targetId=\"%s\" linkText=\"%s\" "
                                     "srcExcl=%d backRef=%d isRefLink=%d isTargetId=%d fallback=%d resolved=%d",
                                     hrefAttr8.c_str(), targetId8.c_str(), linkText8.c_str(),
                                     _srcExcluded?1:0, _isBackRef?1:0, _isRefLink?1:0, _isTargetId?1:0,
                                     allowQuoteFallback?1:0, isFootnoteTarget?1:0);
                    }
                    if (_quoteDbg) {
                        quoteDebugLog("[quote] link=%s id=%s text=%s",
                                      UnicodeToLocal(ldomXPointer(enode, 0).toString()).c_str(),
                                      UnicodeToUtf8(id).c_str(),
                                      quoteDebugPreview(enode->getText(0, 64)).c_str());
                        if (quoteExcluded)
                            quoteDebugLog("[quote] excluded: srcExcl=%d backRef=%d", _srcExcluded?1:0, _isBackRef?1:0);
                        quoteDebugLog("[quote] target=%s fallback=%d resolved=%s",
                                      UnicodeToLocal(ldomXPointer(noteBody, 0).toString()).c_str(),
                                      allowQuoteFallback ? 1 : 0,
                                      isFootnoteTarget ? UnicodeToLocal(ldomXPointer(footnoteContainer, 0).toString()).c_str() : "<none>");
                    }

                    if (isFootnoteTarget) {
                        ldomNode* noteTextNode = footnoteContainer;
                        if (isQuoteFootnotes) {
                            ldomNode* scoped = noteBody;
                            while (scoped && !scoped->isNull()) {
                                if (scoped->isElement()) {
                                    css_style_ref_t st = scoped->getStyle();
                                    if (!st.isNull() && st->display == css_d_block) {
                                        noteTextNode = scoped;
                                        break;
                                    }
                                }
                                if (scoped == footnoteContainer)
                                    break;
                                scoped = scoped->getParentNode();
                            }
                        }
                        lString32 rawNoteText = noteTextNode->getText(U'\n', 4096);
                        if (isQuoteFootnotes) {
                            lString32 firstPara = getFirstParagraphText(rawNoteText);
                            if (!firstPara.empty())
                                rawNoteText = firstPara;
                        }
                        lString32 noteText = normalizeQuoteFootnoteText(rawNoteText);
                        if (_quoteDbg) {
                            quoteDebugLog("[quote] noteNode=%s len=%d preview=%s",
                                          UnicodeToLocal(ldomXPointer(noteTextNode, 0).toString()).c_str(),
                                          noteText.length(),
                                          quoteDebugPreview(noteText).c_str());
                        }

                        if (!noteText.empty()) {
                            LVFontRef noteFont = noteTextNode->getFont();
                            css_style_ref_t noteStyle = noteTextNode->getStyle();
                            ...
                            if (isQuoteFootnotes) {
                                // Quote mode: collect note text to append after current paragraph.
                                // If no collector is provided by caller, just skip inline insertion.
                                if (quote_footnotes) {
                                    quote_footnotes->add(noteText);
                                    if (_quoteDbg)
                                        quoteDebugLog("[quote] collected len=%d", noteText.length());
                                }
                            } else if (isInlineBlockFootnotes) {
                            ...
                            if (isInlineBlockFootnotes) {
                                ...
                                if (noteStyle->line_height.type != css_val_inherited &&
                                    ...
                                    noteLineHeight = lengthToPx(noteTextNode, noteStyle->line_height, noteFontSize, noteFontSize);
                                }

                                // Calculate text-indent for first line
                                int noteIndent = 0;
                                if (noteStyle->text_indent.type != css_val_unspecified) {
                                    noteIndent = lengthToPx(noteTextNode, noteStyle->text_indent, availWidth);
                        ...
                        // For inline modes, skip normal link rendering after injecting note text.
                        // For quote mode, keep rendering the link text itself (like page-bottom mode).
                        if (!isQuoteFootnotes) {
                            ...
                            return; // Skip normal link processing (inline/inline-block only)
                        }
                    } // if (isFootnoteTarget)
                    else if (_quoteDbg) {
                        quoteDebugLog("[quote] skipped unresolved id=%s", UnicodeToUtf8(id).c_str());
                    }
                }
            }
        }
        ...
        if (style->display == css_d_list_item_legacy) { // obsolete (used only when gDOMVersionRequested < 20180524)
            ...
            if (enode->getNodeListMarker(counterValue, marker, marker_width)) {
                ...
                txform->AddSourceLine(marker.c_str(), marker.length(), cl, bgcl, font.get(), lang_cfg, flags | LTEXT_FLAG_OWNTEXT, line_h, valign_dy,
                                      margin, enode);
                // Ensure following normal content starts a fresh paragraph.
                // Otherwise subsequent sibling text may be merged into the quote paragraph
                // and inherit its indentation.
        ...
        if (is_object) { // object element, like <IMG>
            ...
            lString32Collection localQuoteFootnotes;
            lString32Collection* nestedQuoteCollector = quote_footnotes;
            if (rm == erm_final && enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE)) {
                nestedQuoteCollector = &localQuoteFootnotes;
            }
            for (int i = 0; i < cnt; i++) {
                ldomNode* child = enode->getChildNode(i);
                renderFinalBlock(child, txform, fmt, flags, indent, line_h, lang_cfg, valign_dy, is_link_start_p,
                                 nestedQuoteCollector);
            }

            if (nestedQuoteCollector == &localQuoteFootnotes && localQuoteFootnotes.length() > 0) {
                // Quote mode: store collected footnotes for this block; do NOT append to txform.
                // They will be rendered as a separate block (like page_bottom footnotes) for accurate positioning.
                if (rm == erm_final)
                    enode->getDocument()->setQuoteFootnotesForNode(enode, localQuoteFootnotes);
                force_newline_after_quote = true;
                flags &= ~LTEXT_FLAG_NEWLINE & ~LTEXT_SRC_IS_CLEAR_BOTH;
            }
        ...
        // Ensure content following appended quote block starts a fresh paragraph.
        if (force_newline_after_quote)
            flags |= LTEXT_ALIGN_LEFT;
...
void getPageBreakStyle(ldomNode* el, css_page_break_t& before, css_page_break_t& inside, css_page_break_t& after) {
    ...
    while (el) {
        ...
        if ((!firstChild || before != css_pb_auto) && (!lastChild || after != css_pb_auto) && inside != css_pb_auto)
            return;
        ldomNode* parent = el->getParentNode();
        if (!parent)
            return;
...
int measureBorder(ldomNode* enode, int border) {
    ...
    if (border == 0) {
        ...
        if (bw.value == 0 && bw.type > css_val_unspecified)
            return 0; // explicit value of 0: no border
...
int pagebreakhelper(ldomNode* enode, int width) {
    ...
    if (flag == RN_SPLIT_BEFORE_ALWAYS) {
        ...
        while (!node->isNull()) {
            ...
            if (!parent)
                break;
            if (!isFirstBlockChild(parent, node))
                break;
            node = parent;
        }
        ...
        if (top > 0)
            flag = RN_SPLIT_AUTO;
        if ((getPageBreakBefore(enode) == css_pb_always))
            flag = RN_SPLIT_ALWAYS;
    }
    return flag;
}
...
int renderBlockElementLegacy(LVRendPageContext& context, ldomNode* enode, int x, int y, int width, int usable_right_overflow) {
    ...
    if (enode->isElement()) {
        ...
        // Footnote body detection: hide footnote body blocks from the normal document flow.
        // This is needed for page_bottom AND quote modes (quote shows them via renderFinalBlock instead).
        // Only inline/inline_block modes skip this (they inject note text inline).
        if (STYLE_HAS_CR_HINT(style, FOOTNOTE_INPAGE) &&
            ...
            // Priority 1: element's own id (e.g. <p id="fn1" class="footnote">)
            if (enode->hasAttribute(LXML_NS_ANY, attr_id))
                footnoteId = enode->getAttributeValue(LXML_NS_ANY, attr_id);
            // Priority 2: first non-whitespace child is <a id>
            if (footnoteId.empty())
                footnoteId = getFirstChildAnchorId(enode);
            // Priority 3: fallback
            if (footnoteId.empty())
        ...
        */
        ...
        int pb_flag;
        LFormattedTextRef txform;
        LFormattedTextRef quote_txform_legacy;
        {
            ...
            switch (m) {
                ...
                case erm_final: {
                    ...
                    h = enode->renderFinalBlock(txform, &fmt, width - padding_left - padding_right, NULL, &quote_txform_legacy);
                    context.updateRenderProgress(1);
#ifdef DEBUG_DUMP_ENABLED
                    logfile << "\n";
#endif
            ...
            if (context.wantsLines()) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (padding_bottom > 0 && i == count - 1) {
                        pb_flag = margin_bottom > 0 ? RN_SPLIT_AFTER_AUTO : (CssPageBreak2Flags(getPageBreakAfter(enode)) << RN_SPLIT_AFTER);
                    ...
                    if (!isFootNoteBody && enode->getDocument()->getDocFlag(DOC_FLAG_ENABLE_FOOTNOTES) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE_BLOCK) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE)) { // disable for non page-bottom modes
                                ...
                                if (src && src->object) {
                                    ...
                                    if (parent && parent->hasAttribute(LXML_NS_ANY, attr_href)) {
                                        ...
                                        if (sharpPos >= 0 && sharpPos + 1 < (int)href.length())
                                            id = href.substr(sharpPos + 1);
                                        if (!id.empty())
                                            context.addLink(id, link_insert_pos);
                                    }
                                }
                            }
                        }
                ...
                // Quote mode (legacy path): add quote block lines to page splitting context
                if (!quote_txform_legacy.isNull()) {
                    int main_height = 0;
                    for (int qi = 0; qi < count; qi++)
                        main_height += txform->GetLineInfo(qi)->height;
                    int qcount = quote_txform_legacy->GetLineCount();
                    for (int qi = 0; qi < qcount; qi++) {
                        const formatted_line_t* qline = quote_txform_legacy->GetLineInfo(qi);
                        context.AddLine(rect.top + padding_top + main_height + qline->y,
                                        rect.top + padding_top + main_height + qline->y + qline->height, 0);
                    }
                }
            } // wantsLines()
            else {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (!isFootNoteBody && enode->getDocument()->getDocFlag(DOC_FLAG_ENABLE_FOOTNOTES) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE_BLOCK) &&
                        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE)) {
                        // If paragraph is RTL, we are meeting words in the reverse of the reading order:
                        // so, insert each link for this line at the same position, instead of at the end.
                        int link_insert_pos = -1; // append
                        if (line->flags & LTEXT_LINE_PARA_IS_RTL) {
    ...
    void pushVerticalMargin(int next_split_before_flag = RN_SPLIT_AUTO) {
        ...
        if (margin != 0) {
            if (vm_target_node) {
                ...
                if (isMainFlow())
                    printf("pushedVM %d => c_y=%d>%d no target node\n", margin, c_y, c_y + margin);
            }
#endif
    ...
    int getNextFloatMinY(css_clear_t clear) {
        ...
        for (int i = 0; i < _floats.length(); i++) {
            ...
            if (flt->top > y)
                y = flt->top;
            if (clear > css_c_none) {
                if ((clear == css_c_both) || (clear == css_c_left && !flt->is_right) || (clear == css_c_right && flt->is_right)) {
                    if (flt->bottom > y)
                        y = flt->bottom;
                }
            }
...
void renderBlockElementEnhanced(FlowState* flow, ldomNode* enode, int x, int container_width, lUInt32 flags) {
    ...
    // Allow displaying footnote content at the bottom of all pages that contain a link
    // to it, when -cr-hint: footnote-inpage is set on the footnote block container.
    // When DOC_FLAG_FOOTNOTES_INLINE / INLINE_BLOCK is set, footnotes are not in page-bottom mode.
    // When DOC_FLAG_FOOTNOTES_QUOTE is set, we still set isFootNoteBody so the footnote body
    // block can be given zero height (content is shown in the quote under the ref paragraph).
    if (STYLE_HAS_CR_HINT(style, FOOTNOTE_INPAGE) &&
        enode->getDocument()->getDocFlag(DOC_FLAG_ENABLE_FOOTNOTES) &&
        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE) &&
        !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE_BLOCK)) {
        ...
        if (enode->hasAttribute(LXML_NS_ANY, attr_id))
            footnoteId = enode->getAttributeValue(LXML_NS_ANY, attr_id);
        // Priority 2: first non-whitespace child is <a id>
        // (e.g. <p class="fncontent"><a id="fn1" href="...">marker</a>text)
        // Avoids treating body paragraphs as footnote bodies when <a id> is mid-text.
        if (footnoteId.empty())
            footnoteId = getFirstChildAnchorId(enode);
        // Priority 3: fallback - any inner element with id (backward compat for
        // structures like <section class="footnote"><p id="fn1">text</p></section>)
        if (footnoteId.empty())
            footnoteId = enode->getFirstInnerAttributeValue(attr_id);
        if (!footnoteId.empty())
            isFootNoteBody = true;
    ...
    */
    ...
    bool is_floatbox_child = BLOCK_RENDERING(flags, FLOAT_FLOATBOXES) && enode->getParentNode() && enode->getParentNode()->isFloatingBox();
    ...
    if (!flow->isMainFlow())
        printf("\t\t|");
    for (int i = 1; i <= flow->getCurrentLevel(); i++) {
        printf("%c", flow->isMainFlow() ? ' ' : '-');
    }
    printf("%c", m == erm_block ? 'B' : m == erm_table ? 'T'
                                : m == erm_final       ? 'F'
                                                       : '?');
    ...
    switch (m) {
        case erm_table: {
            ...
            if (enode->getParentNode()->isBoxingInlineBox() && enode->getParentNode()->getParentNode() && enode->getParentNode()->getParentNode()->getStyle()->display == css_d_ruby) {
                is_ruby_table = true;
            }
        ...
        case erm_inline: // For inlineBox elements only
        {
            ...
            // In quote mode, footnote body blocks are shown under the ref paragraph; give zero height.
            if (isFootNoteBody && enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE)) {
                fmt.setHeight(0);
                fmt.setWidth(width);
                fmt.push();
                flow->addContentLine(0, 0, 0);
                return;
            }
        ...
        case erm_final: {
            // In quote mode, footnote body blocks are already shown under the ref paragraph;
            // give them zero height so they don't leave a gap in the flow.
            if (isFootNoteBody && enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_QUOTE)) {
                fmt.setHeight(0);
                fmt.setWidth(width);
                fmt.push();
            ...
            LFormattedTextRef quote_txform;
            int final_h = enode->renderFinalBlock(txform, &fmt, inner_width, &float_footprint, &quote_txform);
            ...
            // When a quote block is present (smaller font), tighten the
            // bottom of the block to avoid a disproportionate visual gap
            // between the small-font quote text and the next paragraph.
            int quote_tighten = 0;
            int effective_padding_bottom = padding_bottom;
            if (!quote_txform.isNull()) {
                int qcount = quote_txform->GetLineCount();
                if (qcount > 0) {
                    const formatted_line_t* lastLine = quote_txform->GetLineInfo(qcount - 1);
                    quote_tighten = lastLine->height * 2 / 5;
                }
                effective_padding_bottom = 0;
            }

            int h = padding_top + final_h - quote_tighten + pad_style_h + effective_padding_bottom;
                            ...
                            if (src && src->object) {
                                ...
                                if (parent && parent->hasAttribute(LXML_NS_ANY, attr_href)) {
                                    ...
                                    if (sharpPos >= 0 && sharpPos + 1 < (int)href.length())
                                        id = href.substr(sharpPos + 1);
                                    if (!id.empty())
                                        flow->getPageContext()->addLink(id, link_insert_pos);
                                }
                            }
                        }
                    }
            ...
            // Quote mode: add quote block lines to flow (separate block like page_bottom for accurate positioning)
            if (!quote_txform.isNull()) {
                int qcount = quote_txform->GetLineCount();
                for (int i = 0; i < qcount; i++) {
                    const formatted_line_t* qline = quote_txform->GetLineInfo(i);
                    int lh = qline->height;
                    if (i == qcount - 1)
                        lh -= quote_tighten;
                    flow->addContentLine(lh, 0, qline->baseline);
                }
            }
            ...
            if (effective_padding_bottom > 0) {
                flow->addContentLine(effective_padding_bottom, RN_SPLIT_BEFORE_AVOID, 0, true);
            }
...
void DrawDocument(LVDrawBuf& drawbuf, ldomNode* enode, int x0, int y0, int dx, int dy, int doc_x, int doc_y,
    ...
    if (enode->isElement()) {
        ...
        switch (m) {
            ...
            case erm_final: {
                // In quote mode, footnote body blocks are already shown under the ref paragraph;
                // do not draw them in the main flow.
                bool isFootNoteBody = false;
                if (STYLE_HAS_CR_HINT(style, FOOTNOTE_INPAGE) &&
                    enode->getDocument()->getDocFlag(DOC_FLAG_ENABLE_FOOTNOTES) &&
                    !enode->getDocument()->getDocFlag(DOC_FLAG_FOOTNOTES_INLINE) &&
                ...
                // draw whole node content as single formatted object (main + optional quote block)
                LFormattedTextRef txform;
                LFormattedTextRef quote_txform;
                enode->renderFinalBlock(txform, &fmt, inner_width, NULL, &quote_txform);
                fmt.push();
                {
                    lvRect rc;
                    ...
                    int main_height = 0;
                    for (int i = 0; i < txform->GetLineCount(); i++)
                        main_height += txform->GetLineInfo(i)->height;
                    if (!quote_txform.isNull()) {
                        quote_txform->Draw(&drawbuf, draw_x, draw_y + main_height, marks, nbookmarks);
                    }
                    if (nbookmarks)
                        delete nbookmarks;
                }
...
void setNodeStyle(ldomNode* enode, css_style_ref_t parent_style, LVFontRef parent_font) {
    ...
    } else if ((nodeElementId >= EL_BOXING_START && nodeElementId <= EL_BOXING_END) || nodeElementId == el_pseudoElem || nodeElementId == el_annotation) { // <annotation> is also a FB2 element, so we have to check its parent
...
#define UPDATE_LEN_FIELD(fld)                                                            \
    ...
    switch (pstyle->font_size.type) { // ordered here as most likely to be met
        case css_val_inherited:
    ...
    if (pstyle->pseudo_elem_before_style) {
        if (pstyle->pseudo_elem_before_style->display != css_d_none && pstyle->pseudo_elem_before_style->content.length() > 0 && pstyle->pseudo_elem_before_style->content[0] != U'X') {
    ...
    if (pstyle->pseudo_elem_after_style) {
        if (pstyle->pseudo_elem_after_style->display != css_d_none && pstyle->pseudo_elem_after_style->content.length() > 0 && pstyle->pseudo_elem_after_style->content[0] != U'X') {
            // Not "display: none" and with "content:" different than "none":
            // this pseudo element can be generated
...
// and beware left/right side bearing adjustments...
        ...
        while (true) {
            ...
            for (int i = 0; i < chars_measured; i++) {
                ...
                bool is_cjk = (c >= UNICODE_CJK_IDEOGRAPHS_BEGIN && c <= UNICODE_CJK_IDEOGRAPHS_END && (c <= UNICODE_CJK_PUNCTUATION_HALF_AND_FULL_WIDTH_BEGIN || c >= UNICODE_CJK_PUNCTUATION_HALF_AND_FULL_WIDTH_END));
