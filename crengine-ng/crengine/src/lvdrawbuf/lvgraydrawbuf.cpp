...
static lUInt8 revByteBits1(lUInt8 b) {
    return ((b & 1) << 7) | ((b & 2) << 5) | ((b & 4) << 3) | ((b & 8) << 1) | ((b & 16) >> 1) | ((b & 32) >> 3) | ((b & 64) >> 4) | ((b & 128) >> 5);
}
...
void LVGrayDrawBuf::DrawRotated(LVImageSourceRef img, int x, int y, int width, int height, int rotationAngle) {
    if (rotationAngle == 0) {
    ...
    LVGrayDrawBuf tempBuf(tempW, tempH, _bpp);
        ...
        if (_bpp == 1) {
            for (int x = x0; x < x1; x++) {
                ...
                if (direction == 0 && x % (length1 + length2) < length1)
                    line[x] = color;
                if (direction == 1 && y % (length1 + length2) < length1)
                    line[x] = color;
            }
        } else if (_bpp == 2) {
            for (int x = x0; x < x1; x++) {
