...
bool LZSSUtil::Encode(
    ...
    do {
        ...
        if (match_length <= THRESHOLD) {
            ...
            code_buf[code_buf_ptr++] = (lUInt8)(((match_position >> 4) & 0xf0) | (match_length - (THRESHOLD + 1))); /* Send position and
...
static void readMem(const lUInt8* buf, int offset, lUInt32& dest) {
    dest = ((lUInt32)buf[offset + 3] << 24) | ((lUInt32)buf[offset + 2] << 16) | ((lUInt32)buf[offset + 1] << 8) | buf[offset];
}
...
void WOLWriter::writePageIndex() {
    ...
    *_stream << "<pagetable ver=\"021211 \">";
    ...
    p += pagegroup_1.length() * 4;
    catalog.add(cnv.lsf(p));
    p += pagegroup_1.length() * 4;
    catalog.add(cnv.lsf(p));
    p += pagegroup_delim.length() * 4;
    catalog.add(cnv.lsf(p));
    p += pagegroup_1.length() * 4;
    catalog.add(cnv.lsf(p));
    p += pagegroup_1.length() * 4;
