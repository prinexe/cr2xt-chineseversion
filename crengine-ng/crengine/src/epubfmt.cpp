...
public:
    lString32 _uri;
    lString32 _method;
    EncryptedItem(lString32 uri, lString32 method)
            : _uri(uri)
            , _method(method) {
    }
};
        ...
        for (size_t i = 1; i <= EPUB_ITEM_MAX_ITER; i++) {
            ...
            if (!href.empty() && !id.empty()) {
                ...
                if (isEpub3 && navHref.empty()) {
                    ...
                    if (properties == U"nav" || properties.startsWith(U"nav ") || properties.endsWith(U" nav") || properties.pos(U" nav ") >= 0) {
                        navHref = href;
                    }
                }
