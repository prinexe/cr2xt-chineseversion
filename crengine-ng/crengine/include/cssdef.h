...
enum css_orphans_widows_value_t
{ // supported only if in range 1-9
...
#define CSS_CR_HINT_FOOTNOTE_IGNORE 0x08000000 // -cr-hint: footnote-ignore block is not a footnote (even if \
                                               //                           everything else indicates it is)
#define CSS_CR_HINT_FOOTNOTE_INLINE 0x10000000 // -cr-hint: footnote-inline marks element for inline footnote styling
...
typedef struct css_length_tag
{
    ...
    lUInt32 pack() {
        return (lUInt32)type + (((lUInt32)value) << 4);
    }
} css_length_t;
