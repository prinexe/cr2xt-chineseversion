...
public:
    ...
    bool has(int offset, int page) {
        _mutex.lock();
        for (int i = 0; i < 2; i++) {
            if (_items[i]._valid && ((_items[i]._offset == offset && offset != -1) || (_items[i]._page == page && page != -1))) {
                return true;
            }
        }
        return false;
...
public:
    ...
    ldomWordEx* getSelectedWord() {
        return _words.getSelWord();
    }
    // access to words
    ldomWordExList& getWords() {
        return _words;
    }
...
enum
{
    ...
    PGHDR_CHAPTER_NAME = 1024,  // show chapter name in center
    PGHDR_CHAPTER_PAGE_NUMBER = 2048,  // show chapter page number
    PGHDR_CHAPTER_PAGE_COUNT = 4096    // show chapter page count
};
...
private:
    ...
    inline bool isPageMode() const {
        return m_view_mode == DVM_PAGES;
    }
    inline bool isScrollMode() const {
        return m_view_mode == DVM_SCROLL;
    }
...
public:
    ...
    void setStatusMode(int pos, bool showClock, bool showTitle, bool showBattery, bool showBatteryPercent, bool showNavbar, bool showChapterMarks, bool showPosPercent, bool showPageNumber, bool showPageCount, bool showChapterName = false, bool showChapterPageNumber = false, bool showChapterPageCount = false);
    ...
    void checkPos();
    LVFontRef getBatteryFont() const {
        return m_batteryFont;
    }
    void setBatteryFont(LVFontRef font) {
        m_batteryFont = font;
    }
    ...
    void close();
    /// set buffer format
    void setDrawBufferBits(int bits) {
        m_drawBufferBits = bits;
    }
    ...
    ldomXPointer getCursorPos() const {
        return m_cursorPos;
    }
    /// set cursor position
    void setCursorPos(ldomXPointer ptr) {
        m_cursorPos = ptr;
    }
    ...
    ldomMarkedRangeList* getMarkedRanges() {
        return &m_markRanges;
    }
    ...
    bool getShowCover() const {
        return m_showCover;
    }
    /// sets coverpage display flag
    void setShowCover(bool show) {
        m_showCover = show;
    }
    ...
    void setDefaultCover(LVImageSourceRef cover) {
        m_defaultCover = cover;
        clearImageCache();
    }

    /// get background image
    LVImageSourceRef getBackgroundImage() const {
    ...
    // callback functions
    /// set callback
    LVDocViewCallback* setCallback(LVDocViewCallback* callback) {
        LVDocViewCallback* old = m_callback;
        m_callback = callback;
        return old;
    }
    /// get callback
    LVDocViewCallback* getCallback() {
        return m_callback;
    }
    ...
    doc_format_t getDocFormat() const {
        return m_doc_format;
    }
    ...
    lString8 getDefaultFontFace() const {
        return m_defaultFontFace;
    }
    /// set default font face
    void setDefaultFontFace(const lString8& newFace);
    /// returns status bar font face
    lString8 getStatusFontFace() const {
        return m_statusFontFace;
    }
    ...
    cr_rotate_angle_t GetRotateAngle() const {
    ...
    int getBatteryChargingConn() const {
        return m_battery_charging_conn;
    }
    /// returns battery charge level
    int getBatteryChargeLevel() const {
        return m_battery_charge_level;
    }
    ...
    int getPageheaderPosition() const {
        return m_pageHeaderPos;
    }
    ...
    lUInt32 getBackgroundColor() const {
    ...
    const LVScrollInfo* getScrollInfo() {
        updateScroll();
        return &m_scrollinfo;
    }
    ...
    int getRequestedFontSize() const {
        return m_requested_font_size;
    }
    ...
    int getStatusFontSize() const {
        return m_status_font_size;
    }
